...
const fields={
  pairs:$("pairs"),serverEnabled:$("serverEnabled"),serverUrl:$("serverUrl"),imageModel:$("imageModel"),videoModel:$("videoModel"),
  imageConcurrency:$("imageConcurrency"),videoConcurrency:$("videoConcurrency"),
  ...
  aspectRatio:$("aspectRatio"),imageOutputs:$("imageOutputs"),videoDuration:$("videoDuration"),videoOutputs:$("videoOutputs"),
...
async function saveForm(){const o={};for(const [k,v] of Object.entries(fields))o[k]=v.type==='checkbox'?v.checked:v.value;o.serverUrl='ws://127.0.0.1:3000/ws/flow';await chrome.storage.local.set({flowPairAutoForm:o});if(kickServerFields.has(document.activeElement?.id))chrome.runtime.sendMessage({type:'FLOW_SERVER_RECONNECT'}).catch(()=>{});}
async function loadForm(){const {flowPairAutoForm:o}=await chrome.storage.local.get('flowPairAutoForm');if(o)for(const [k,v] of Object.entries(o))if(fields[k]&&v!=null&&k!=='serverUrl'){if(fields[k].type==='checkbox')fields[k].checked=Boolean(v);else fields[k].value=v;}fields.serverUrl.value='ws://127.0.0.1:3000/ws/flow';}
async function ensureConcurrencyPreset1459(){/* V2.8.5: server jobs receive GLOBAL concurrency from V2.8; never overwrite saved values here. */}
...
const kickServerFields=new Set(['serverEnabled']);
...
run.addEventListener('click',async()=>{try{
  if(fields.imageModel.value==='NONE'&&fields.videoModel.value==='NONE')throw new Error('Image model và Video model không thể cùng là None.');
  ...
  const response=await chrome.runtime.sendMessage({type:'FLOW_RUN_PAIRS',tabId:tab.id,pairs:fields.pairs.value,options:{
    imageModel:fields.imageModel.value,videoModel:fields.videoModel.value,
    imageConcurrency:Number(fields.imageConcurrency.value||9),videoConcurrency:Number(fields.videoConcurrency.value||4),
    ...
    aspectRatio:fields.aspectRatio.value,imageOutputs:fields.imageOutputs.value,videoDuration:fields.videoDuration.value,videoOutputs:fields.videoOutputs.value,
...
(async()=>{await ensureConcurrencyPreset1459();await loadForm();await loadServerStatus();const {flowPairAutoUiTab}=await chrome.storage.local.get('flowPairAutoUiTab');if(flowPairAutoUiTab&&document.getElementById(flowPairAutoUiTab))await activateTab(flowPairAutoUiTab,false);await loadRuntime();})();
